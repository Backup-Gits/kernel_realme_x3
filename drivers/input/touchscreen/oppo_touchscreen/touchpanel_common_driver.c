...
extern int get_esd_check_happened(void);
...
static void init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
  ...
  //cs gpio
  ts->hw_res.tp_cs_gpio = of_get_named_gpio(np, "tp-cs-gpio", 0);
  if (gpio_is_valid(ts->hw_res.tp_cs_gpio)) {
	  rc = gpio_request(ts->hw_res.tp_cs_gpio, "tp-cs-gpio");
	  if (rc)
		  TPD_INFO("unable to request gpio [%d]\n", ts->hw_res.tp_cs_gpio);
  } else {
	  TPD_INFO("ts->tp_cs_gpio not specified\n");
  }
...
//Liuchao@BSP.TP.Basic , 2020/03/02, Add for pull down cs under esd check
   if(get_esd_check_happened()){
    TPD_INFO("%s: esd_check:%d.\n", __func__,get_esd_check_happened());	
    msleep(250);
   }
...
static void lcd_trigger_load_tp_fw(struct work_struct *work)
{
    ...
    if (ts->lcd_trigger_load_tp_fw_support) {
        if (is_running) {
            ...
            TPD_INFO("%s start\n", __func__);
            is_running = true;
	//Liuchao@BSP.TP.Basic , 2020/03/12, Add for tRT2BRST>=10ms
		msleep(10);
            mutex_lock(&ts->mutex);
            ts->ts_ops->reset(ts->chip_data);
            mutex_unlock(&ts->mutex);
