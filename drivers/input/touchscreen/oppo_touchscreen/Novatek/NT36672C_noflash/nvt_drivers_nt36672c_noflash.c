...
static int32_t Write_Partition(struct chip_data_nt36672c *chip_info, const u8 *fwdata, size_t fwsize)
{
    ...
    for (list = 0; list < chip_info->partition; list++) {
        ...
        for (i = 0 ; i < count ; i++) {
			...
			CTP_SPI_WRITE_FIRMWARE(chip_info->s_client, chip_info->fwbuf, len + 1);
#if NVT_DUMP_SRAM
...
#if NVT_FW_BUF_DESTROYED_WORKAROUND
			if ((chip_info->g_fw_buf_backup) && (chip_info->g_fw_backup_ready == true)){
				TPD_INFO("use backup fw\n");
				request_fw_headfile->size = chip_info->g_fw_len_backup;
				request_fw_headfile->data = chip_info->g_fw_buf_backup;
				fw = request_fw_headfile;
				//check fw again for print log
				nvt_check_bin_checksum(fw->data, fw->size);
			}
#endif
        }
    } else {
...
#if NVT_FW_BUF_DESTROYED_WORKAROUND
		if (chip_info->g_fw_buf_backup) {
			TPD_INFO("backup checksum ok fw\n");
			chip_info->g_fw_len_backup = fw->size;
			memcpy(chip_info->g_fw_buf_backup, fw->data, fw->size);
			chip_info->g_fw_backup_ready = true;
		}
#endif
    }
...
#if NVT_FW_BUF_DESTROYED_WORKAROUND
    chip_info->g_fw_buf_backup = vmalloc(FW_BUF_SIZE);

    if (chip_info->g_fw_buf_backup == NULL) {
        TPD_INFO("fw buf vmalloc error\n");
        //ret = -ENOMEM;
        goto err_g_fw_buf_backup;
    }
    chip_info->g_fw_backup_ready = false;
#endif
...
#endif
    {
	TPD_INFO("In Recovery mode,%d %d %d\n",(ts->boot_mode == MSM_BOOT_MODE__RECOVERY),is_oem_unlocked(),ts->fw_update_in_probe_with_headfile);
...
#if NVT_FW_BUF_DESTROYED_WORKAROUND
    if (chip_info->g_fw_buf_backup) {
        vfree(chip_info->g_fw_buf_backup);
    }
err_g_fw_buf_backup:
#endif
    if (chip_info->g_fw_buf) {
