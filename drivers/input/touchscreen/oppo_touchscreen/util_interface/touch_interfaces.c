...
*******************************************************/
int32_t spi_read_write_lagecy(struct spi_device *client, uint8_t *buf, size_t len, uint8_t *rbuf, SPI_RW rw)
{
    struct spi_message m;
    struct spi_transfer t = {
        .len    = len,
...
int32_t CTP_SPI_WRITE_FIRMWARE(struct spi_device *client, uint8_t *buf, uint16_t len)
{
    int32_t ret = -1;
    int32_t retries = 0;
    ...
    while (retries < 5) {
        ret = spi_read_write_lagecy(client, buf, len, NULL, SPIWRITE);
        if (ret == 0)	break;
        retries++;
    }
